...
protected func GetAmmoCraftingMaximum(itemRecord: wref<Item_Record>) -> Int32 {
  ...
  let invQuant: Int32 = 0;
  let max: Int32 = 0;
  
  // TODO: Get max values programatically somehow?
  if itemRecord.TagsContains(n"HandgunAmmo") {
    invQuant = trans.GetItemQuantityByTag(player, n"HandgunAmmo");
    max = 500;              
  }
  if itemRecord.TagsContains(n"RifleAmmo")   {
    invQuant = trans.GetItemQuantityByTag(player, n"RifleAmmo");
    max = 700;
  }
  if itemRecord.TagsContains(n"ShotgunAmmo") {
    invQuant = trans.GetItemQuantityByTag(player, n"ShotgunAmmo");
    max = 100;
  }
  if itemRecord.TagsContains(n"SniperAmmo")  {
    invQuant = trans.GetItemQuantityByTag(player, n"SniperAmmo");
    max = 100;
  }

  return Max(0, (max - invQuant + itemRecipe.amount - 1) / itemRecipe.amount);
}
